	...
	o.general_queryUIElementTooltipData = function(entityId, elementId, elementOwner)
	{
		...
		case "EIMO.RepairButton":
			...
			},
			{
				...
				text = ::MSU.String.capitalizeFirst(::EIMO.Mod.ModSettings.getSetting("setForSale")) + " on items to mark their type for sale"
			},
			{
				id = 4,
				type = "hint",
				icon = "ui/icons/EIMO_mouse_right_button_ctrl_shift.png",
				text = ::MSU.String.capitalizeFirst(::EIMO.Mod.ModSettings.getSetting("setFavorite")) + " on items to mark them as favorite (they will then not be sold)"
			}];
