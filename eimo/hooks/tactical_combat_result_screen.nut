	...
	o.eimo_onSmartLootButtonPressed <- function()
	{
		...
		local function onItemTaken(i, idx)
		{
			...
			if (i != null && i.getItemType() < ::Const.Items.ItemType.Ammo)
			{
				...
				else
				{
					...
					else if (::EIMO.getSalvageRatio(i) < ::EIMO.Mod.ModSettings.getSetting(::EIMO.SalvageThresholdID).getValue())
					{
		...
		if (li < lootis.len()) // if we couldn't take all the items...
		{
			...
			local function isEligible(i)
			{
				...
				if (i.eimo_isFavorite() || i.eimo_isIDFavorite()) return false; //Keep favorite items from EIMO
				return true;
			}
