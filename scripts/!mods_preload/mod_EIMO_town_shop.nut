this.getroottable().Const.EIMO.hookTownShop <- function()
{
			...
			if (!this.Tactical.isActive())
			{
				...
				for( local i = this.World.Assets.getStash().getCapacity() - 1; i >= 0; i = --i )
				{
					if (this.Stash.getItemAtIndex(i).item != null)
					{
						...
						if (this.World.Flags.has(this.Const.EIMO.getItemSaleFlag(item)) && this.World.Flags.get(this.Const.EIMO.getItemSaleFlag(item)) == 1 && !item.isFavorite() && !(item.getCondition() < item.getConditionMax() && dratio > this.Const.EIMO.SellThreshold))
						{
